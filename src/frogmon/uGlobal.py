...
class GLOB:
	...
	def appendTIMELAPsControlInfo(fileNM, Interval_sec, Resolution_x, Resolution_y, Rotation, Start_time, End_time):
		rc = -1
		try :
			with open(fileNM, 'r') as f:
				json_data = json.load(f)	
				json_data['INTERVAL']     = Interval_sec
				json_data['RES_X']        = Resolution_x
			...
			print("[appendTIMELAPsControlInfo ERROR]: %s" % e)
